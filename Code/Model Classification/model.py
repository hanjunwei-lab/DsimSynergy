...
class FCNN(torch.nn.Module):
    ...
    def forward(self, Drug1, Drug2, GO1, GO2, ATC1, ATC2, cell_embed, index):
        ...
        self.last_layer_feature = embed
        embed = self.relu(embed)
        embed = self.dropout(embed) 
        embed = self.embed_fc4(embed)

        return embed, self.last_layer_feature
...
class Synergy(torch.nn.Module):
    ...
    def forward(self,drug_set, cell_set ,GO_adj, GO_weight, ATC_adj, ATC_weight, drug_smiles_fea, index):
        ...
        synergy_score, last_layer_feature = self.FCNN(Drug1, Drug2, GO1, GO2, ATC1, ATC2, cell_embed, index)  
        feature = [drug_embed, cell_embed, go_embed, atc_embed, last_layer_feature, torch.cat((Drug1,Drug2,GO1,GO2,ATC1,ATC2,cell_exp[index[:, 2],:]),1)]
